...
std::pair<std::string, std::string> g_latestMessage;
std::mutex g_latestMessageMutex;
...
void OnPXREAClientCallback(void* context, PXREAClientCallbackType type, int status, void* userData)
{
    ...
    case PXREADeviceStateJson:
        ...
        std::lock_guard<std::mutex> lock(g_latestMessageMutex);
        g_latestMessage = {dsj.devID, dsj.stateJson};
...
std::pair<std::string, std::string> get_latest_message() {
    std::lock_guard<std::mutex> lock(g_latestMessageMutex);
    return g_latestMessage;
}
...
PYBIND11_MODULE(xrobotoolkit_sdk, m) {
    ...
    m.def("get_latest_message", &get_latest_message,
          "Returns the latest message as a (device_id, json_string) tuple. "
          "Note: new messages overwrite previous ones.");
