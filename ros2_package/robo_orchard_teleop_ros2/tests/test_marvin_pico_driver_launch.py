...
def _load_module():
    ...
    try:
        spec = importlib.util.spec_from_file_location(
            "marvin_pico_driver_launch", LAUNCH_PATH
        )
...
def test_marvin_launch_starts_driver_node_directly():
    description = _load_module().generate_launch_description()

    driver = next(
        entity
        for entity in description.entities
        if entity.kwargs.get("executable") == "marvin_driver_node"
    )
    assert driver.kwargs["package"] == "robo_orchard_marvin_ros2"
    assert driver.kwargs["name"] == "marvin_driver_node"
    assert driver.kwargs["parameters"][0].name == "driver_config_file"
    overrides = driver.kwargs["parameters"][1]
    assert overrides["auto_enable_side"].name == "auto_enable_side"
    assert overrides["auto_enable_mode"].name == "auto_enable_mode"
